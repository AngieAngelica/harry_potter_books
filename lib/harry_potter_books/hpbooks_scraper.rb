...
class HarryPotterBooks::Harry
  ...
  def hpbooks1to7_list
    hpbooks_list.each do |book|
      @@hpbooks1to7<< {
        :name => book.css("itemprop=name").text,
        :link => book.css('a').attribute('href').value
                      }
    end
  end

  def self.get_info(book_url)
    individual_hp_book = "https://www.goodreads.com" + url

    doc = self.get_page(individual_hp_book)

    hpbook_name = doc.css("h1").text.delete("\n") # or hpbook_name = doc.at('title').text
    hpbook_link = doc.css("h1").text.delete("\n") # or hpbook_name = doc.at('title').text
    hpbook_summary = doc.css("span")[47].inner_text.strip #.strip removes unnecessary characters from text
    hpbook_rating = doc.css("span")[40].inner_text
    hpbook_quote = doc.css("span")[-3].text
